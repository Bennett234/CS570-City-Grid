...
public class Intersection {
    ...
    public enum TrafficState {
        ...
        private TrafficState next;

        static {
            PHASE_1.next = PHASE_2;
            PHASE_2.next = PHASE_3;
            PHASE_3.next = PHASE_4;
            PHASE_4.next = PHASE_1;
        }
    ...
    public int timeUntilAvailable(Direction from, Direction to) {
        assert from != to;
        assert isAvailable(to);

        int count = 0;
        TrafficState state = this.state;

        while (!canDoDuringPhase(state, from, to)) {
            count++;
            state = state.next();
        }
